...
- **ui/ audit (2026-09-25): every component diffed against live registry; all kept as-is.** Registry tabs/toggle/toggle-group/separator/switch now use `data-active:` / `data-horizontal:` / `data-checked:` selectors, but installed bits-ui emits `data-state` / `data-orientation` (different names — dead regardless of value form), so regenerating those breaks active/selected styling silently. Static components (badge/card/empty/skeleton) differ only by import order. Do not bulk-update ui/ until a bits-ui release emits the new attribute names.
